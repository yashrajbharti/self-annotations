(() => {
    const {Controller: t, UI: e} = window.MINDAR.IMAGE;
    AFRAME.registerSystem("mindar-image-system", {
        ...
        tick: function() {},
        setup: function({imageTargetSrc: t, maxTrack: i, showStats: a, uiLoading: s, uiScanning: n, uiError: o, captureRegion: r}) {
        ...
        _startVideo: function() {
            ...
            }).then((t => {this.video.addEventListener("loadedmetadata", (() => {this.video.setAttribute("width", this.video.videoWidth), this.video.setAttribute("height", this.video.videoHeight), this._startAR()})), this.video.srcObject = t})).catch((t => {console.log("getUserMedia error", t), this.el.emit("arError", {
                    error: "VIDEO_FAIL"
                })}))
        },
        _startAR: async function() {
            ...
            n > i.clientWidth / i.clientHeight ? (s = i.clientHeight, a = s * n) : (a = i.clientWidth, s = a / n), this.controller = new t({
                ...
                onUpdate: t => {if ("processDone" === t.type)
                        this.mainStats && this.mainStats.update();
                    else if ("updateMatrix" === t.type) {
                        const {targetIndex: e, worldMatrix: i} = t;
            ...
            const {dimensions: g} = await this.controller.addImageTargets(this.imageTargetSrc);
            for (let t = 0; t < this.anchorEntities.length; t++) {
                const {el: e, targetIndex: i} = this.anchorEntities[t];
        ...
        init: function() {
            ...
            }), this.data.autoStart && this.el.sceneEl.addEventListener("renderstart", (() => {t.start()}))
        }
        ...
        init: function() {
            ...
            e && e.getAttribute("mindar-image-paint") && e.addEventListener("model-loaded", (() => {e.getObject3D("mesh").traverse((t => {t.isMesh && t.material && t.material.name === e.getAttribute("mindar-image-paint") && (this.paintMaterial = t.material)}))})), t.visible = !1, t.matrixAutoUpdate = !1
        },
        setupMarker([t, e]) {
        ...
        updatePaint(t) {
            if (!this.paintMaterial || this.el.object3D.visible)
                return;
            const e = t.length,
                i = t[0].length,
            ...
            (new THREE.TextureLoader).load(r, (t => {this.paintMaterial.map.dispose(), this.paintMaterial.map = t, this.paintMaterial.needsUpdate = !0}))
        }
    })
})();
